 ...
 * 
 */
public class AlgoHUOP_P1P2P3P4 {
...
//	long totalUtility = 0; // use the long
	...
	// A map to store the TWU of each item
...
//		mapItemToTWUO = new HashMap<Integer, Double>();		
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				double tu = 0; // transaction utiliy
...
//					// add the uo(X,T_q) of the item in the current transaction to
//					// its TWUO
...
//					System.out.println("Tid="+ tid + "  tu="+ mapOfTU.get(tid) + ";  item="+ pair.item + "   uo=" +pair.utility/mapOfTU.get(tid)+ "   ruo=" +rutil/mapOfTU.get(tid));
		...
		// Test, for each item
//		for (utilityList Px : listOfutilityLists) {
//			System.out.println(Px.item+":¡¡sup= " +Px.support+", uo= "+Px.sumUO
//					+",  uo/sup= "+Px.getsumUO()+", ruo= "+Px.sumRUO+",  ruo/sup= "+Px.getsumRUO());
//		}
...
//		double o1 = mapItemToTWUO.get(item1);
//		double o2 = mapItemToTWUO.get(item2);
//		if(o1 < o2) { 
//            return -1;
//        }else if(o1 > o2) {
//            return 1;
...
//		Double o1 = mapItemToTWUO.get(item2);
//		Double o2 = mapItemToTWUO.get(item1);
//		if(o1 < o2) { 
//            return 1;
//        }else if(o1 > o2) {
//            return -1;
...
//        }
	...
	private void HUOP_Miner(int[] prefix, UO_List pUL, List<UO_List> ULs, double minUtilityOccu) throws IOException {
		...
		for (int i = 0; i < ULs.size(); i++) {
            ...
            System.out.println();
			String itemset="";
			for(int kk=0; kk<prefix.length; kk++){
				itemset = itemset+":" + prefix[kk];
			}
			System.out.println(itemset+"   " + X.item + " X.sup= " + X.support + " , occuOfX=" + X.getAvegUO());
			...
			if(X.support >= (int) Math.ceil(minSupport * tid)){				
				...
				if (occuOfX >= minUtilityOccu) {
					// save to file
					writeOut(prefix, X.item, X.support, occuOfX, maxOccuOfX);
				}
//				if(prefix.length == 0){
//					System.out.println("item="+ X.item + " sumUO="+X.sumUO +" AvegUO="+X.getAvegUO()+ " sumRUO="+X.sumRUO +" AvegRUO="+X.getAvegRUO());
//				}
					...
					for (int j = i + 1; j < ULs.size(); j++) {
						...
						if(temp != null && temp.support >= minSupport * tid) {
						   exULs.add(temp);
					    	...
					    	if(temp != null){
					    	//	System.out.println("P3..."+itemset+ "  "+X.item+"-"+Y.item +"   "+temp.support);
					    	}							    	
					    }
			    ...
			    //	System.out.println("P2..."+ itemset +"   maxOccuOfX="+ maxOccuOfX);
			    }
			}else{
			//	System.out.println("***P1***"+itemset+"- " + X.item + " X.sup= " + X.support + " , occuOfX=" + X.getAvegUO());
		    }
...
//            System.out.println("maxOccu= "+ String.format("%.6f", minUtilityOccu) + "  ---"+topNValue[i]);
	...
	/**
	 * This method constructs the UO-list of pXY «å«¹
	 * 
	 * @param P
	...
	private UO_List construct(UO_List P, UO_List px, UO_List py) {
		...
		for (Element ex : px.elements) {
			...
			if (P == null) {
				...
				Element e = findElementWithTID(P, ex.tid); // ****** «é
				if (e != null) {
					// Create new element
					// ============================================================================
	 ...
	 */
	private void writeOut(int[] prefix, int item, int sup, double uo, double maxOccuOfX) throws IOException {
		...
		buffer.append(", upper-bound= " + String.format("%.6f", maxOccuOfX));
	...
	public void printStats() {		
		System.out.println("=============  HUOP (P1 + P2 + P3 + P4) - STATS =============");
